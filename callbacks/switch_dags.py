...
async def switch_dags(call: CallbackQuery, session: AsyncSession, bot: Bot):
    ...
    for dag in enumerate(res_json['dags']):
        ...
        if dag[1]['dag_id'] == dag_id and not dag[1]['is_paused']:
            ic("не в паузед")
            try:
                response = requests.patch(url=f'{HOST_AIRFLOW}/api/v1/dags/cleanup_xcom?update_mask=is_paused',headers={'Content-Type': 'application/json'}, auth=(USER_AIRFLOW, PASSWORD_AIRFLOW), json={'is_paused': True})
                if response.ok:
                    
                    # await bot.delete_message(chat_id=call.message.from_user.id, message_id = call.message.message_id-1)
                    await call.message.answer(f'DAG {dag_id} поставлен на паузу')
                    await call.answer()
                else: 
        ...
        if dag[1]['dag_id'] == dag_id and dag[1]['is_paused']:
            ic("в паузед")
            try:
                response = requests.patch(url=f'{HOST_AIRFLOW}/api/v1/dags/cleanup_xcom?update_mask=is_paused',headers={'Content-Type': 'application/json'}, auth=(USER_AIRFLOW, PASSWORD_AIRFLOW), json={'is_paused': False})
                if response.ok:
                    await call.message.answer(f'DAG {dag_id} снят с паузы')
                    await call.answer()
                else: 
                    await call.message.answer(f'Ошибка {response.status_code}')
                    await call.answer()
